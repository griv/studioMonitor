...
// Step 1 keeps the old whole-document endpoints working so admin.html is not
// touched during the migration. Step 2 deletes all of this.

function configDoc() {
  const doc = { banks: {} };
  for (const bank of listBanksForAdmin()) {
    const items = {};
    for (const item of bank.items) {
      if (item.enabled && !item.fit) continue;   // the file only ever recorded deviations
      items[item.name] = { enabled: item.enabled, fit: item.fit };
    }
    // A bank with nothing to say never appeared in the file either.
    if (!bank.defaultFit && !Object.keys(items).length) continue;
    doc.banks[bank.name] = { defaultFit: bank.defaultFit, items };
  }
  return doc;
}
...
function vibesDoc() {
  return {
    vibes: listVibes().map(v => ({
      name: v.name,
      banks: v.sources.filter(s => s.type === 'bank').map(s => s.value),
      dwellTime: v.dwellTime ?? undefined,
      transitionDuration: v.transitionDuration ?? undefined,
      shuffle: v.shuffle ?? undefined,
      defaultFit: v.defaultFit ?? undefined,
    })),
  };
}

function applyVibesDoc(doc) {
  const db = getDb();
  tx(() => {
    db.prepare('DELETE FROM vibe').run();
    for (const v of doc?.vibes || []) {
      upsertVibeInner(db, v);
    }
    bumpLibraryVersion();
  });
}
...
module.exports = {
  ...
  getVibeByName, patchItem, patchBank, upsertVibe, deleteVibe, upsertGroup, deleteGroup,
  configDoc, applyConfigDoc, vibesDoc, applyVibesDoc, seedFromLegacyJSON,
  PLAYABLE, ITEM_ORDER,
};
