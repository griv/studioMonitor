...
function toItem(row, { forAdmin = false } = {}) {
  if (!forAdmin) {
    return {
      ...
      fit: row.fit || null,
      objectPosition: row.object_position || null,   // an explicit crop, when one was chosen
...
function listVibes() {
  ...
  return vibes.map(v => ({
    ...
    videoCrossfadeMs: v.video_crossfade_ms,
...
// The only path that removes an item row. scanLibrary deliberately never does —
// see the rules at the top of scan.js — but an explicit delete has to, or the item
// returns as a MISSING tile on the next scan and can never be cleared.
function deleteItem(id) {
  const res = getDb().prepare('DELETE FROM item WHERE id = ?').run(id);
  if (res.changes) bumpLibraryVersion();
  return res.changes > 0;
}
...
function upsertVibe({ id, name, sources = [], dwellTime, transitionDuration, videoCrossfadeMs,
                      shuffle, defaultFit, captionMode, captionHoldMs }) {
  const db = getDb();
  return tx(() => {
    const values = [name, dwellTime ?? null, transitionDuration ?? null, videoCrossfadeMs ?? null,
                    shuffle === null || shuffle === undefined ? null : (shuffle ? 1 : 0),
                    defaultFit ?? null, captionMode ?? null, captionHoldMs ?? null];

    let vibeId = id;
    if (vibeId) {
      db.prepare(`UPDATE vibe SET name = ?, dwell_ms = ?, transition_ms = ?, video_crossfade_ms = ?,
                  shuffle = ?, default_fit = ?, caption_mode = ?, caption_hold_ms = ? WHERE id = ?`)
        .run(...values, vibeId);
    } else {
      vibeId = Number(db.prepare(`INSERT INTO vibe (name, dwell_ms, transition_ms, video_crossfade_ms,
                                  shuffle, default_fit, caption_mode, caption_hold_ms)
...
function upsertVibeInner(db, v) {
  const vibeId = Number(db.prepare(`INSERT INTO vibe (name, dwell_ms, transition_ms, video_crossfade_ms,
                                    shuffle, default_fit, caption_mode, caption_hold_ms)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(v.name, v.dwellTime ?? null, v.transitionDuration ?? null, v.videoCrossfadeMs ?? null,
...
module.exports = {
  ...
  patchItem, deleteItem, patchBank, upsertVibe, deleteVibe, upsertGroup, deleteGroup,
