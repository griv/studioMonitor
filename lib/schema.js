...
const MIGRATIONS = [
  ...
  // Videos need their own transition number. A still crossfades between two fixed
  // frames, so the duration is pure taste; a video crossfade has to *start before
  // the clip ends*, which makes the same number also a lead time — and the length
  // that looks right for stills is rarely the length worth giving up off the end
  // of a clip. NULL inherits the session setting, like every other vibe override.
  db => db.exec(`ALTER TABLE vibe ADD COLUMN video_crossfade_ms INTEGER`),
];
